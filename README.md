...
- `mainbbong@gmail.com`으로 로그인하면 선택한 날짜에 기록을 남긴 모든 사용자의 이메일, 완료 여부와 운동 결과를 읽기 전용 관리자 패널에서 확인할 수 있습니다.
...
- 전체 사용자 기록은 이메일 허용 목록을 서버 측에서 다시 검사하는 `get_admin_daily_records` 함수로만 조회하며, 현재는 `mainbbong@gmail.com` 한 계정만 허용합니다.
