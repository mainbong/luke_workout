...
add constraint workout_sessions_details_object_check
check (jsonb_typeof(details) = 'object');

alter table public.routine_completions
add column details jsonb not null default '{}'::jsonb,
add constraint routine_completions_details_object_check
check (jsonb_typeof(details) = 'object');
...
check (
  (workout_type = 'pushup' and (set_count is null or set_count between 1 and 5))
...
alter table public.workout_sessions
alter column program_version_id drop default;
...
begin
  ...
  if new.program_version_id = 'luke-weekly-2026-09-02' then
    ...
    },
    {
      ...
      "exercises": [
        ...
        },
        {
          ...
          "note": "예: 5, 5, 3, 3, 3, 3, 2, 2, 2, 2회 = 총 30회, 10세트 성공입니다."
        },
        {
          "name": "4. 다음 풀업 데이",
          "prescription": "10세트 이내 +10회 · 11세트 이상 유지",
          "note": "10세트 이내 완료하면 다음 풀업 데이 목표가 {{nextPullTarget}}회로 증가하고, 11세트 이상이면 같은 {{pullTarget}}회 목표를 유지합니다. 전체 세트 수를 반드시 기록합니다."
        },
        {
          "name": "러닝머신",
          "prescription": "속도 7 이상 · 15분",
          "note": "풀업 미션 뒤 진행합니다. 15분 동안 유지한 속도를 기록합니다.",
          "guides": [
    ...
    },
    {
      ...
      "exercises": [
        ...
        },
        {
          ...
          "note": "성공하면 다음 목요일 목표가 {{nextPushTarget}}회로 증가하고, 실패하면 {{pushTarget}}회를 유지합니다. 로그인 사용자별 기록만 사용합니다."
        },
        {
          "name": "5. 5세트 전 조기 성공",
          "prescription": "4세트 이내 성공 +20회",
          "note": "5세트로 나눠서 할 필요는 없습니다. 5세트 전에 끝나면 다음 목요일 목표는 {{nextPushEarlyTarget}}회를 적용합니다. 예: 100회 목표 시 25, 25, 25, 25회로 4세트만에 끝난다면 다음 주 목표는 120회입니다."
        },
        {
          "name": "플랭크 타바타",
          "prescription": "{{plankHoldSeconds}}초 Hold / {{plankRestSeconds}}초 휴식 × 3세트",
          "note": "TABATA 타이머를 켜두고 3세트 진행합니다. 3세트 성공 시 다음 운동에서 Hold는 10초, 휴식은 5초씩 증가합니다. {{plankHoldSeconds}}/{{plankRestSeconds}}초로 3세트 성공 시 다음은 {{nextPlankHoldSeconds}}/{{nextPlankRestSeconds}}초로 3세트 도전합니다.",
          "guides": [
    ...
    },
    {
      ...
      "exercises": [
        ...
        },
        {
          ...
          "note": "1세트 Max Reps 진행, 2, 3세트는 Max Reps의 50% 개수만 진행합니다. 딥스는 자세가 안 좋으면 부상을 입기 쉽습니다. 반드시 쇼츠·동영상 가이드로 안전하고 정확한 자세를 머리로 숙달 후 운동을 수행합니다. 수행 중 어깨의 찝힘 같은 불편감이 있다면 동작 수행을 멈추고 Push Up으로 전환하되, 팔꿈치를 옆구리에 완전히 붙이고 삼두에 집중하는 푸쉬업으로 변경해서 동일하게 3세트 진행합니다.",
          "guides": [
        ...
        },
        {
          ...
          "note": "어 이렇게 가벼워도 되나? 정도의 무게, 4kg 정도부터 추천하고 무게를 점차 올립니다. 3세트까진 무게를 올리고 15회 진행이 안 된다면 이후에 내리면서 진행합니다. 다양한 동작 가이드가 있으나 중요한 것은 통증이 없고, 승모에 힘이 덜 들어가는 본인만의 진행 방향·견갑 상태를 찾고 진행하는 것입니다. 보통 견갑을 바깥으로 빼고 진행해야 승모에 힘이 덜 들어갑니다. 완전한 수평 레이즈보다 앞으로 살짝 기울어진 방향은 수평 레이즈 시 통증이 있다면 완화시켜줄 수 있습니다. 팔을 올릴 땐 빠르게, 내려올 때 천천히 하는 것에 집중합니다. 존나 털렸을 땐 반동으로라도 덤벨을 올리되 내려올 땐 천천히 버티면서 내려오는 것에 집중합니다.",
          "guides": [
        ...
        },
        {
          ...
          "note": "이미 어깨 가슴 다 털려서 더할 필요가 있나 싶은 수준의 상태입니다. 가벼운 무게로 비교적 빠른 속도로 진행합니다. 무게 기준은 그날의 멘탈이 시키는 대로 하시되 20회씩 채워야만 합니다. 입에서 호흡음이 새어나가며 이빨 사이로 침이 뿜어져 나오며 진행하게 되는 마지막 세트임을 명심하고 전력질주합니다.",
          "guides": [
        ...
        },
        {
          ...
          "note": "웨이트 루틴을 마친 뒤 진행합니다. 운동이 고됐으므로 러닝은 5분만 타고 쿨다운해줍니다. 속도는 10을 시작으로 올릴 수 있을 만큼 올려서 진행하고 매주 기록합니다. 5분 동안 유지한 최대 속도를 0.1 단위로 입력합니다.",
          "guides": [
      ...
      "exercises": [
        {
          ...
          "note": "각 세트는 맨몸으로 목표 횟수에 도전합니다. 실패하면 풀업머신으로 전환해 해당 세트 목표를 채웁니다. 5세트 모두 맨몸으로 성공하면 다음 일요일부터 세트당 1회 증가하고, 한 세트라도 머신 보조를 사용하면 유지합니다.",
          "guides": [
        ...
        {
          "name": "이두컬 머신",
          "prescription": "한쪽 10회 × 3세트",
          "note": "이두컬 머신 중 아무거나 택해서 진행합니다. RPE 7~8 무게로 시작합니다. 10회에 실패해도 감량 없이 최대 노력으로 수행합니다. 안 올라가면 6회에 3세트를 마무리해도 됩니다. 각 팔의 횟수는 동일하게 가져가는 것이 좋기 때문에 한쪽 팔이 더 많이 수행했다면 반대쪽 팔도 맞춰서 수행합니다.",
          "guides": [
